...
test('renders reset filters in column menu', async () => {
  const { getByText } = setup();
  userEvent.click(getByText('Сбросить фильтры'));
  await waitFor(() =>
...
describe('for main menu', () => {
  ...
  test('renders reset filters in main menu', async () => {
    const { getByText } = setup({ ...mockedProps, isMain: true });
    userEvent.click(getByText('Сбросить фильтры'));
    await waitFor(() =>
      expect(mockGridApi.setFilterModel).toHaveBeenCalledWith(null),
