...
> = {
  ...
  Calendar: [
    ...
    {
      value: 'previous calendar quarter',
      label: t('\u041a\u0432\u0430\u0440\u0442\u0430\u043b'),
    },
...
const formatMonthTitleRu = (value: Dayjs) =>
  `${value.locale('ru').format('MMMM')} ${value.format('YYYY')}`.replace(
    /^./,
...
const formatInputDate = (value: Dayjs | null, includeTime = false) =>
  value
    ? value.format(includeTime ? DATE_TIME_INPUT_FORMAT : DATE_INPUT_FORMAT)
    : '';
...
const sanitizeInputValue = (value: string, includeTime: boolean) =>
  formatMaskedInputValue(value.replace(/\D/g, ''), includeTime);
...
const resolveInitialMode = (
  ...
  const isSpecificDateTimeRange =
    decodeSpecificDateTimeRange(value).matchedFlag;
...
export function CalendarRangeFrame(props: FrameComponentProps) {
  ...
  const onInputChange =
    (inputType: 'start' | 'end') => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <Wrapper data-test={DateFilterTestKey.CalendarV2Frame}>
      <Header $mode={mode}>
        <NavButton buttonStyle="link" $hidden>
