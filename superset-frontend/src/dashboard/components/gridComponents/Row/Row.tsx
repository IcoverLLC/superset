...
} from 'src/dashboard/util/collapsibleRows';
...
const GridRow = styled.div<{ editMode: boolean; isCollapsed: boolean }>`
...
const collapsedRowContentStyles = (theme: SupersetTheme) => css`
...
const Row = memo((props: RowProps) => {
  ...
  const [isRowCollapsed, setIsRowCollapsed] = useState(() =>
    getInitialCollapsedStateForRow({
      dashboardId,
      rowComponent: rowComponent as LayoutItem,
    }),
  );
  ...
  /* eslint-disable react-you-might-not-need-an-effect/no-derived-state,
     ...
     synchronize persisted per-dashboard collapse state when the row identity or metadata changes */
  useEffect(() => {
    setIsRowCollapsed(
      getInitialCollapsedStateForRow({
        dashboardId,
  ...
  const handleChangeCollapseEnabled = useCallback(
    (event: { target: { checked: boolean } }) => {
      ...
      if (!enableCollapse) {
        ...
        setCollapsedRowForDashboard({
          dashboardId,
          rowId: rowComponent.id as string,
          collapsed: false,
        });
      }
    },
  ...
  const canCollapse =
    isCollapsibleRow(rowComponent as LayoutItem) && rowItems.length > 0;
    ...
    ({ dragSourceRef }: { dragSourceRef: RefObject<HTMLDivElement> }) => (
      ...
      >
        ...
        >
          ...
          {isCollapsed ? (
            <div css={collapsedRowContentStyles as any}>
              {t('Collapsed row')}
            </div>
          ) : (
            <>
