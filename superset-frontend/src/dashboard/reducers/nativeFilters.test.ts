...
test('SET_NATIVE_FILTERS_CONFIG_COMPLETE removes deleted filters from state', () => {
  ...
  // filter2 was deleted: it is absent from the payload and listed explicitly
  const action = {
    type: SET_NATIVE_FILTERS_CONFIG_COMPLETE as typeof SET_NATIVE_FILTERS_CONFIG_COMPLETE,
    filterChanges: [
...
test('SET_NATIVE_FILTERS_CONFIG_COMPLETE preserves filters omitted from a partial payload', () => {
  const initialState = {
    filters: {
...
test('SET_NATIVE_FILTERS_CONFIG_COMPLETE merges updates and explicit deletions', () => {
  const initialState = {
    filters: {
      filter1: createMockFilter('filter1', [1, 2], ['tab1']),
      filter2: createMockFilter('filter2', [3, 4], ['tab2']),
  ...
  // filter1 is untouched, filter2 and customization1 are updated, filter3 is deleted
  const action = {
    type: SET_NATIVE_FILTERS_CONFIG_COMPLETE as typeof SET_NATIVE_FILTERS_CONFIG_COMPLETE,
    filterChanges: [
